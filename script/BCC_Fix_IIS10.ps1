...
"Next # 01.07 Ensure WebDav feature is disabled"
...
"Finished #1.07"
"----------------------------------------"
 "Next #02.03 Ensure forms authentication require SSL""
...
# 02.03
"#02.03  Ensure forms authentication require SSL"

foreach ($site in $sites) {
    Write-Log "Starting #2.03 for site: $site"
    "Starting #2.03 for site: $site"
	...
	"#2.03 Hardened for site: $site"
    } else {
        Write-Log "#2.03 Already Hardened for site: $site"
        "#2.03 Already Hardened for site: $site"
    }
...
Get-WebConfigurationProperty -pspath "MACHINE/WEBROOT/APPHOST/$site" -filter 'system.web/authentication/forms' -name 'requireSSL' | Format-Table -AutoSize
    ...
    Write-Log "----------------------------------------"
    "Finished #2.03 for site: $site"
    "----------------------------------------"
}

"Next #2.04 Ensure forms authentication is set to use cookies"

pause

# 02.04
"#02.04 Ensure forms authentication is set to use cookies"

foreach ($site in $sites) {
    Write-Log "Starting #2.04 for site: $site"
    "Starting #2.04 for site: $site"
    ...
    if ($currentValue -ne 'UseCookies') {
        ...
        "#2.04 Hardened for site: $site"
    } else {
        Write-Log "#2.04 Already Hardened for site: $site"
        "#2.04 Already Hardened for site: $site"
    }
    ...
    Write-Log "----------------------------------------"
   "Finished #2.04 for site: $site"
   "----------------------------------------"
}

"Next #2.05 Ensure 'cookie protection mode' is configured for forms authentication"

pause

# 02.05
Write-Log "Starting #2.05 Ensure 'cookie protection mode' is configured for forms authentication"
"#2.05 Ensure 'cookie protection mode' is configured for forms authentication"
...
"Finished #2.05"
"----------------------------------------"
"Next #03.06 Ensure httpcookie mode is configured for session state"

pause

# 03.06
Write-Log "Starting #3.06"
"#3.06  Ensure httpcookie mode is configured for session state"
...
"Finished #3.06 for site: $site"
"----------------------------------------"
}

"Next #03.12 Ensure Server Header is removed"
...
"Next #04.01 Ensure 'maxAllowedContentLength' is configured"
...
"Next #04.02 Ensure 'maxURL request filter' is configured"

pause

# 04.02
Write-Log "Starting #4.02 Ensure 'maxURL request filter' is configured"
"#4.02 Ensure 'maxURL request filter' is configured"
...
"Next #04.03 Ensure MaxQueryString request filter is configured"

pause

# 04.03
Write-Log "Starting #4.03  Ensure MaxQueryString request filter is configured"
"#4.03  Ensure MaxQueryString request filter is configured"
...
"Next #04.04 Ensure non-ASCII characters in URLs are not allowed"
...
"Next 04.05 Ensure Double-Encoded requests will be rejected"

pause

# 04.05
Write-Log "Starting #4.05 Ensure Double-Encoded requests will be rejected"
"#4.05 Ensure non-ASCII characters in URLs are not allowed"
...
"Finished #04.05"
"----------------------------------------"
"Next #07.05 Ensure TLS 1.1 is Disabled"
...
if ($currentValueServerEnabled.Enabled -ne 0 -or $currentValueServerDisabledByDefault.DisabledByDefault -ne 1) {
    ...
    "#7.05 Hardened (Server)"
} else {
    Write-Log "#7.05 Already Hardened (Server)"
    "#7.05 Already Hardened (Server)"
}
...
if ($currentValueClientEnabled.Enabled -ne 0 -or $currentValueClientDisabledByDefault.DisabledByDefault -ne 1) {
    ...
    "#7.05 Hardened (Client)"
} else {
    Write-Log "#7.05 Already Hardened (Client)"
    "#7.05 Already Hardened (Client)"
}
...
"Next #07.12 Ensure TLS Cipher Suite ordering is Configured"
